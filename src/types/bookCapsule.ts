...
export interface BookCapsule {
  ...
  /** Коли користувач переглянув капсулу вже ПІСЛЯ настання `reopenAt` (п.19 ТЗ — "viewed" стан;
   * повноцінний "completed recall" — окрема майбутня Фаза Recall, тут лише підготовлена
   * точка інтеграції). `null`, доки не переглянуто (або поки `reopenAt` ще не настав). */
