 ...
 * типом кандидата (той самий "кожна фіча не ділить дрібну ЧИСТУ логіку з іншою" принцип, що й
 * приватні копії `trimOrNull` у `beforeAfter.ts`/`loreEntity.ts`/`dnfReflection.ts`).
 *
 * ПОПЕРЕДНЄ ФОРМУЛЮВАННЯ цього коментаря посилалось на `seasonCardFile.ts`/
 * `fingerprintCardFile.ts` як на приклад того самого принципу. Тих файлів більше немає:
 * POLYTSIA V1.7, Phase 5 (ТЗ §13/§98) звела їх в один `shareCardFile.ts`. Це не суперечність —
 * там дублювалась ІНТЕГРАЦІЯ з нативним API й політика дозволів (три однакові правки на кожну
 * зміну поведінки), тут же — чиста функція ранжування над власним типом. Докладніше про межу
 * між цими випадками — шапка `src/lib/shareCardFile.ts`.
 * ---------------------------------------------------------------------------------------- */
