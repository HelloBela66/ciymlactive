 ...
 *
 * Видима лише поки книга РЕАЛЬНО читається — "Читаю" або "Перечитую" (`canEditPreReadingReflection`,
 * REREADING MODEL Фаза 9: перечитування тепер отримує власну нотатку "До" для свого `reading_run`,
 * `docs/READING_RUN.md`) — після завершення форма ховається: писати "до" заднім числом, уже
 * знаючи фінал, підважило б сам сенс порівняння До/Після на Book Memory
 * (`docs/BEFORE_AFTER.md` §Відомі обмеження). Уже збережена нотатка лишається видимою (лише
 * для читання) — форма редагування ж доступна тільки поки умова вище виконується.
 */
