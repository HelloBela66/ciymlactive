...
| zustand | **Прибрано з `package.json` (POLYTSIA V1.6.2, #170).** Було в залежностях від M0 як заготовка "UI/session стан", але жодного `*Store.ts`/`create(`-виклику так і не з'явилось за весь V1/V1.5/V1.6 — уся клієнтська/серверна логіка стану реально йде через `@tanstack/react-query`+`useState` вище. Знайдено як неприбрану залежність двома незалежними аудитами (`docs/V1_6_FULL_AUDIT_REPORT.md` T10, повторено `docs/V1_6_1_FULL_AUDIT_REPORT.md`) — цей рядок лишається лише як історична примітка. | — |
