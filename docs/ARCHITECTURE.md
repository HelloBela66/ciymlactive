...
**Оновлено POLYTSIA V1.5, Фаза 14 (CLEANUP), доповнено після POLYTSIA V1.6 (усі 22 фази) і
POLYTSIA V1.6.1 (усі 28 фаз, `docs/V1_6_1_FINAL_REPORT.md`)** — попередня версія цього розділу
була записаною на Milestone 0 ЗАДУМАНОЮ структурою ("book/[userBookId].tsx", "session/setup.tsx",
"note/editor.tsx" тощо), яка ніколи не відповідала реальному `app/` і розійшлась із ним іще на
Milestone 1-2. Нижче — фактичний перелік, звірений напряму зі структурою на пристрої власника
продукту (`device_list_dir`, а не з пам'яті чи зі старішого документа), 47 файлів маршрутів
(44 на кінець V1.6 + 3 нових консолідаційних хаби V1.6.1: `next-read.tsx` Фаза 14,
`my-reading.tsx` Фаза 15, `reread-comparison/[workId].tsx` Фаза 12):
...
│   ├── index.tsx                    # ГОЛОВНА: поточне читання, вхід у щоденник/"Що почитати завтра?"
│   ├── library/
│   │   └── index.tsx                # БІБЛІОТЕКА: полиці (10 ілюстрованих тем) + фільтр статусу + список
│   ├── calendar.tsx                 # КАЛЕНДАР: місяць-сітка з позначкою днів із сесіями
│   ├── search.tsx                   # ПОШУК: спільний каталог + кураторська добірка + Google Books + ISBNdb
│   └── profile/
│       └── index.tsx                # ПРОФІЛЬ: статистика + входи в цілі/нагадування/TBR/Wrapped/
│                                     #   бекап/історію/перевірку даних
├── backup.tsx                       # Резервна копія: експорт/відновлення JSON + стан копіювання (Фаза 13)
├── capsule/
│   ├── [workId].tsx                 # «Капсула книги»: перегляд (POLYTSIA V1.6, Фаза 4)
│   └── [workId]/
│       └── edit.tsx                 # «Капсула книги»: створення/редагування, один екран
├── completion/
│   └── [workId].tsx                 # Підсумок одразу після завершення книги (статистика+темп+дати)
├── cover-photo/
│   └── [editionId].tsx              # Камера/галерея → обкладинка (для будь-якої книги)
...
├── import/
│   ├── goodreads.tsx                # Імпорт бібліотеки з Goodreads CSV
│   └── review.tsx                   # Перегляд імпорту перед збереженням
...
│                                     #   V1.6.1 Фаза 16 — розширено до 5-секційного хабу
├── my-reading.tsx                   # «Моє читання»: спільний хаб аналітики (V1.6.1, Фаза 15) —
│                                     #   5 екранів (Статистика/Wrapped/Сезони/Відбиток/TBR),
│                                     #   що раніше не мали спільної точки входу
├── next-read.tsx                    # «Що читати далі?»: спільний хаб рекомендацій (V1.6.1,
│                                     #   Фаза 14) — Primary (Обери мені/TBR) + Secondary
│                                     #   (Завтра/Тренди), замінив 3 незалежні картки на Home
...
├── reread-comparison/
│   └── [workId].tsx                 # «Як змінилася книга для тебе»: порівняння ≥2 завершених
│                                     #   reading_run книги (V1.6.1, Фаза 12, REREADING MODEL)
├── seasons/
│   └── [seasonKey].tsx              # Завершені книги конкретного сезону (Зима/Весна/Літо/Осінь)
├── series/
│   └── [seriesId].tsx               # Усі твори серії за порядком
├── session/
│   ├── [sessionId].tsx              # Активна сесія: таймер, пауза, швидкий запис щоденника, завершення
│   └── launch/
│       └── [userBookId].tsx         # Запуск сесії (сторінка/ціль)
├── shelf/
│   ├── [shelfId].tsx                # Вміст полиці
│   └── new.tsx                      # Створення полиці: назва, опис, тема оформлення
├── statistics.tsx                   # Повна статистика
├── tbr.tsx                          # TBR reality check
├── tomorrow.tsx                     # «Що почитати завтра?»: жанр+час+мета → одна книга
├── trends.tsx                       # «Тренди»: топ за кількістю користувачів, що додали собі
├── work/
│   ├── [workId].tsx                 # Book Details: статус, "є фізично", оцінка, серія, щоденник книги
│   └── new.tsx                      # Ручне додавання книги (12+ полів)
└── wrapped/
    └── [year].tsx                   # Річний Wrapped
```

Це навмисно ПЛОСКІ сегменти (`backup.tsx`, `history.tsx`, `data-doctor.tsx`, `statistics.tsx`,
`tbr.tsx`, `tomorrow.tsx`, `trends.tsx` — усі прямо в `app/`, а не вкладені під `profile/` чи
`settings/`, як планувалось на Milestone 0): кожен новий екран за 14 фаз додавався туди, де
Expo Router найпростіше типізує маршрут одразу, без попереднього рефакторингу структури —
навмисне рішення "не ускладнюй навігаційне дерево заради самого дерева", а не недогляд.
...
необов'язковими полями, без ланцюжка модалок (`app/session/[sessionId].tsx`).
...
POLYTSIA V1.6.1 додала ще три: `OfflineNotice` (Фаза 20, банер відсутності інтернету — реальної
досяжності, не лише статусу з'єднання, `useIsOffline`), `OnboardingHintCard` +
`useOnboardingHint` (Фаза 18, одноразові контекстні підказки — не туторіал), `CardPreviewText`
(Фаза 22, фіксоване масштабування шрифту для 3 shareable PNG-карток, де системний Dynamic Type
ламав компоновку).
...
| 10 | Обкладинки книг усюди в застосунку (Home/Бібліотека/Пошук/Book Details/Календар/Полиці/Wrapped), обкладинка в результатах пошуку (локальний каталог + провайдери), "Додати обкладинку" (камера/галерея) для книг без офіційної обкладинки + публікація фото в спільний каталог | 8 (спільний каталог), 9 | Готово, перевірено на реальному пристрої |
...
Після M11 (build, відкладено) власник продукту продовжив функціональну роботу окремим
16-фазним milestone'ом "POLYTSIA V1.5 — PERSONAL READING OS" (Фази 1-16, ведеться поза цією
таблицею — узагальнений підсумок кожної фази йде в `CHANGELOG.md`, а не тут, щоб ця таблиця
лишалась стабільним історичним записом M0-M11).
...
| expo-network | Реальна досяжність інтернету, не лише стан з'єднання (V1.6.1, Фаза 20, `useIsOffline`) | SDK-сумісна версія, резолвиться `npx expo install expo-network` — не звірено з `registry.npmjs.org` вручну (той самий підхід, що й `expo-image-picker` рядком нижче) |
...
| zustand | Точковий UI/сесійний стан (див. рядок "Клієнтський стан" вище) — реально використовується в `src/stores/importDraftStore.ts` (передача "сирого" результату пошуку між `app/(tabs)/search.tsx`/`app/isbn-scan.tsx` і `app/import/review.tsx`). **POLYTSIA V1.6.2, #170 спершу помилково позначив цю залежність як неприбрану й видалив з `package.json`** — перевірка велась по неповній локальній копії репозиторію без `src/stores/`; CI (`Typecheck, lint, tests`) одразу це впіймав, залежність повернуто в тому ж #170. Докладніше — `CHANGELOG.md` (запис #170). | ^5.0.15 |
