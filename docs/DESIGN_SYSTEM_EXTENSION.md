...
- **successful scan** — на момент написання цього документа помилково вважалось нереалізованим
  ("у застосунку немає фічі сканування штрихкоду/ISBN камерою") — фактична помилка: сканування
  ISBN камерою (`app/isbn-scan.tsx`, `expo-camera`) існує з Milestone 7 (2026-09-07), за 5 днів
  до написання цього документа (2026-09-12); просто ніхто не підключив сюди виклик. Виправлено
  в POLYTSIA V1.6.1, Фазі 23 (`docs/HAPTICS_CLEANUP.md`) — рівно один виклик одразу після того,
  як відсканований штрихкод проходить перевірку контрольної цифри ISBN, до початку пошуку книги
  (локальна база/каталог/провайдери можуть і не знайти видання — це вже не сам скан).
