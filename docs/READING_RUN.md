...
## Свідомо ПОЗА межами Фази 11

Підключення інших сутностей до `reading_run` лишається окремою наступною фазою:

| Фаза | Що підключається |
| --- | --- |
| 12 | Rereading UX + порівняння прочитань — агрегований показ "як читалося цього разу vs минулого разу", використовуючи `run_number`; тут з'явиться екран історії спогадів/нотаток "До"/капсул/DNF-знімків по всіх run книги |
