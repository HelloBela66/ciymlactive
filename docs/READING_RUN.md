...
> **ОНОВЛЕНО P0 FIX, POLYTSIA V1.6.2, Фаза 1:** абзац вище описував стан ДО цієї фази — фолбек
> справді НЕ чіпав `user_book.status`, і саме це виявилось реальним, а не лише теоретичним,
> дефектом (ТЗ V1.6.2, розділи 5-6, `docs/V1_6_2...` фінальний звіт): кнопка "Почати читання" —
> єдиний шлях, яким реально користуються `ReadingControls`/`SessionLaunchScreen` — залишала книгу
> з активним `reading_run`, але статусом `want_to_read`/`finished`/`did_not_finish`, що йому
> суперечить (`dataIntegrityDoctor.ts`, `legacy_contradictory_status` — перевірка існувала й ДО
> цієї фази, просто хибно вважала такий стан можливим лише для legacy-даних до Фази 7). Тепер
> `start()` ТАКОЖ виставляє статус атомарно в тій самій транзакції (`inferStatusForSessionStart`
> у самому репозиторії): активний run визначає, чи це продовження проходу (`reading`/`rereading`
> без змін, чи відновлення з `paused` за номером run), відсутність активного run — старт НОВОГО
> проходу: `finished` → `rereading` (той самий цільовий статус, що й явна кнопка "Перечитати
> книгу" — книга вже раз дочитана, це свідомий повторний прохід); `did_not_finish` → `reading`,
> НЕ `rereading` (рішення власника продукту, 2026-09-14): книга ще ЖОДНОГО разу не була дочитана,
> тож кнопка на Book Details для неї показує "Дочитати" (не "Почати читання") — це продовження
> того самого незавершеного наміру, не перечитування; будь-який інший стан (найчастіше
> `want_to_read`) → `reading`. `UserBookRepository.updateStatus` лишається окремим, незміненим
> шляхом для явної зміни статусу через чіп.

> **ДОДАНО READING RUN CANCEL/DISCARD FIX, POLYTSIA V1.6.2, Фаза 2:** P0 FIX Фази 1 вище зробив
> видимою суміжну прогалину, що існувала й раніше, але без реальних наслідків: `discard()` (кнопка
> "Скасувати сесію" на активному екрані читання, `app/session/[sessionId].tsx`) досі чіпав ЛИШЕ
> саму сесію. Run, який щойно створив (або відновив) виклик `start()`, лишався `in_progress`
> НАЗАВЖДИ, без жодної живої сесії всередині — це вже була задокументована "graceful"-властивість
> моделі (run без UNIQUE-гейту в схемі), але тепер, коли `start()` ще й змінює `user_book.status`
> (Фаза 1), той самий сценарій "тапнув 'Почати читання' не на ту книжку, одразу 'Скасувати сесію'"
> лишав книгу видимо позначеною "Читаю"/"Перечитую" в Бібліотеці й на Головній — без жодного
> способу це відмінити з UI, окрім прямого редагування бази.
>
> Фікс — `computeStatusAfterRunRemoval()`, дзеркальна логіка до `inferStatusForSessionStart()`:
> якщо ПІСЛЯ soft-delete скасованої сесії її run лишається без жодної іншої живої сесії, і сам run
> ще НІКОЛИ не завершувався (`finishedAt == null`) — це була відмінена спроба, а не реальна
> історія, тож `discard()` тепер, в тій самій транзакції: (1) м'яко видаляє й сам run
> (`ReadingRunRepository.discard`, `run_number` не перевикористовується — та сама гарантія, що й
> усюди в цій сутності); (2) обчислює, яким статус книги мав би бути, якби цього run ніколи не
> існувало, за найновішим ЖИВИМ run, що лишився (`getLatestByUserBookId`): немає жодного —
> `want_to_read` (і `started_at` теж повертається в `null` — `want_to_read` з непорожнім
> `started_at` сам по собі порушення, яке ловить `dataIntegrityDoctor.ts`,
> `want_to_read_with_started_at`); найновіший лишився `finished`/`did_not_finish` — статус книги
> повертається саме туди (не в `reading`); інакше — `reading`/`rereading` за номером проходу, той
> самий принцип, що й у `inferStatusForSessionStart`. Якщо в run лишились інші живі сесії
> (звичайне "скасував одну сесію посеред багатосесійного читання") або run уже завершений — ні run,
> ні статус НЕ чіпаються: це реальна історія, а не помилковий старт. Легасі-сесії без
> `reading_run_id` (до Фази 7) — як і раніше, лише сама сесія позначається видаленою.
...
Так само НЕ нова фаза REREADING MODEL, а частина ТЗ Фази 27 ("migration safety tests + backup
format update"), яка стосується безпосередньо `reading_run` і тому документується тут, а не
лише в `CHANGELOG.md`.

### Спільний backfill-модуль (`src/data/db/legacyRunBackfill.ts`)

ЧАСТИНА 2 (JS backfill-цикл) кожної з шести міграцій ланцюга (`020_reading_run_backfill.ts`/
`021_book_memory_run.ts`/`022_pre_reading_reflection_run.ts`/`023_book_capsule_run.ts`/
`024_dnf_reflection_run.ts`/`025_rating_run.ts`) винесена в новий спільний файл — той самий
алгоритм, дослівно, без жодної зміни запитів чи пріоритету вибору run (сама міграція тепер лише
викликає відповідну функцію). Причина — Restore нижче, якому потрібна РІВНО ТА САМА логіка поза
контекстом одноразової міграції.

### Restore — `reading_run` тепер у бекапі, старі бекапи отримують "compatible legacy ReadingRun"

Дві незалежні знахідки при перевірці бекапу проти REREADING MODEL:

1. **`reading_run` НІКОЛИ не входила в `BACKUP_TABLE_ORDER`** (`BackupRepository.ts`) — з моменту
   появи таблиці (Фаза 6, `019_reading_run.ts`) аж до Фази 27 включно. Реальна прогалина: жоден
   експортований файл бекапу не містив історії перечитувань користувача взагалі. Виправлено —
   `reading_run` додана одразу після `user_book` (той самий "батьки перед дітьми" порядок, що й
   решта списку).
2. **Відновлення СТАРОГО бекапу (зробленого до Фази 6) відтворює ситуацію "до міграції 020-025"**
   — сесії/спогади/капсули/рейтинги/нотатки "До"/DNF-знімки в файлі є, `reading_run`/
   `reading_run_id` — ні. Але схемні міграції виконуються РІВНО ОДИН РАЗ у житті БД (сама БД уже
   на `LATEST_SCHEMA_VERSION` задовго до restore) — вони НЕ перезапускаються при кожному
   `restoreAll`. Без додаткового кроку користувач, що відновив старий бекап, одразу побачив би в
   «Перевірці даних» (Фаза 26) `session_without_run` для КОЖНОЇ книги — штучну проблему, створену
   самою процедурою відновлення.

Рішення: `useRestoreBackup` (`src/features/backup/useBackup.ts`) одразу після
`BackupRepository.restoreAll` викликає `backfillAllLegacyReadingRunLinks`
(`legacyRunBackfill.ts`) — оркестратор, що застосовує всі чотири функції у порядку залежностей
(спершу `reading_run`, потім усе, що на неї лінкується). Той самий "збій допоміжного кроку не
позначає весь restore невдалим" підхід, що й уже існуючий `rebuildCapsuleRemindersAsync` поруч
(Фаза 4).

**Свідомо НЕ всередині `BackupRepository.restoreAll`**: `restoreAll` — навмисно ЧИСТА дзеркальна
вставка (`BackupRepository.test.ts`, round-trip тест: "жодне поле не перегенерується"). Backfill
СТВОРЮЄ нові рядки (`reading_run`) і МІНЯЄ існуючі (`reading_run_id`) — усередині `restoreAll` це
порушило б інваріант round-trip тесту. Тому backfill — окремий крок над нею, той самий рівень,
що й `rebuildCapsuleRemindersAsync`.

**Чому це безпечно викликати ПОВТОРНО** (на відміну від самих міграцій, що виконуються раз):
кожна з чотирьох функцій фільтрує рівно ті рядки, які ще не мають зв'язку — виклик на вже
узгодженій БД (напр. після відновлення СВІЖОГО бекапу, де `reading_run` вже заповнена) — чистий
no-op. `backfillLegacyReadingRuns` додатково перевіряє `NOT EXISTS` по `reading_run` (оригінальна
міграція 020 цього не потребувала — вона виконувалась одразу після 019 на порожній таблиці).

### Migration safety net (`migrationRunner.test.ts`)

До Фази 27 кожен тест на конкретну міграцію (009-027) стартував рівно з одного кроку "до неї"
(`__applyMigrationsForTests(db, N-1)`), з мінімальним seed, побудованим саме під ту одну
міграцію — вичерпно доводить коректність кожної окремо, але ніколи не перевіряв, що ВЕСЬ
ланцюжок компонується разом на реалістичній бібліотеці за ОДИН виклик `migrateDbIfNeeded`. Дві
нові пари тестів закривають це: "V1.5 baseline (migration 011) → LATEST_SCHEMA_VERSION" (16
міграцій за один стрибок) і "V1.6 baseline (migration 018) → LATEST_SCHEMA_VERSION" (9
міграцій — увесь ланцюжок REREADING MODEL 019-025 разом, на книгах із капсулою/спогадом/
нотаткою "До"/DNF-знімком/рейтингом одночасно).
