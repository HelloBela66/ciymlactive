...
## Свідомо ПОЗА межами Фази 10

Підключення інших сутностей до `reading_run` (DNF, порівняння прочитань) лишається окремими
наступними фазами:

| Фаза | Що підключається |
| --- | --- |
| 11 | DNF — `dnf_reflection` прив'язується до конкретного run, що не дочитали |
| 12 | Rereading UX + порівняння прочитань — агрегований показ "як читалося цього разу vs минулого разу", використовуючи `run_number`; тут з'явиться екран історії спогадів/нотаток "До"/капсул по всіх run книги |
