# PERSONAL_LORE.md — «Персонажі» (Фаза 9) → «Світ книги» (Фаза 10)

POLYTSIA V1.6, Фаза 9 (CHARACTERS), схема уніфікована одразу під Фазу 10 (PERSONAL LORE); Фаза
10 додала лише UI для решти трьох типів — без нової міграції. Цей документ фіксує точну
поведінку фічі й архітектурне рішення, яке інакше довелось би пояснювати заново при кожній фазі
(той самий підхід, що й `docs/BOOK_CAPSULES.md`/`docs/RECALL.md`/`docs/BEFORE_AFTER.md`/
`docs/READING_EXPERIENCE_TIMELINE.md`/`docs/STALE_READING_RECAP.md`).
...
рецензія — лише те, що читач сам занотував про персонажа під час чи після читання. ТЗ Фази 10
розширює це до всього "світу книги": Персонаж, Місце, Термін, Організація — один і той самий
принцип для всіх чотирьох. ТЗ прямо забороняє: "Не роби NLP entity extraction" — жодного
автоматичного визначення сутностей з тексту книги, жодного зовнішнього джерела. Користувач додає
й пов'язує елементи лору вручну.

## Тип

Фаза 9 запровадила лише `type: 'character'` в UI; Фаза 10 додає вибір типу при створенні —
Персонаж / Місце / Термін / Організація, рівно порядок і назви з ТЗ (`LORE_ENTITY_TYPE_ORDER`,
`src/design/loreEntityType.ts`). Тип незмінний після створення (`UpdateLoreEntityInput` його не
приймає) — той самий "клас поля" підхід, що й `BookCapsule`'s `userBookId`. Реакція
(`LoreEntityReactionId`) лишається UI-специфічною лише для персонажів (ТЗ Фази 9: "Не
використовуй жорстку універсальну classification" — реакція не універсальна для всього лору) —
секція "Ставлення до персонажа" на екрані деталей рендериться лише коли `entity.type ===
'character'`.

## Точка входу

Компактна картка `LoreSection` (у Фазі 9 звалась `CharactersSection`) — кількість уже доданих
елементів лору і кнопка "Відкрити світ книги" — на ДВОХ екранах (ТЗ Фази 9: "На Book Details/
Memory: «Персонажі»"; ТЗ Фази 10: "Назва section: «Світ книги»"):
...
Обидва навмисно ведуть на той самий екран `app/lore/[workId].tsx` (у Фазі 9 —
`app/characters/[workId].tsx`, перейменовано разом із секцією у Фазі 10 для узгодженості з
рештою кодової бази — `LoreEntity`/`useLoreEntities`/`LoreEntityRepository`), той самий
"компактна картка + посилання на власний екран" підхід, що й `BookCapsuleSection`. Картка
продубльована локально на кожному екрані (той самий house-патерн, що й `RevisitLaterEntryLine`/
...
6. **Жодного NLP.** Схема не передбачає "джерела" чи "впевненості" визначення — полів, які
   натякали б на автоматичне визначення; користувач додає й пов'язує елементи лору вручну через
   `app/lore/[workId]/[entityId].tsx`.

### Екрани

- `app/lore/[workId].tsx` — список усіх елементів лору твору (усі типи разом, найновіші зверху)
  + форма швидкого додавання: вибір типу (`ChipSelect`), ім'я, коротка примітка, сторінка
  першої появи.
- `app/lore/[workId]/[entityId].tsx` — деталі елемента: редагування імені/опису, чипи реакції
  (лише для персонажів), перемикач "обране", ручний список зв'язаних записів щоденника
  (чекбокс-пікер із усього щоденника книги), видалення. Сам елемент знаходиться фільтром по вже
