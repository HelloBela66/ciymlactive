...
const { isValidIsbn, isbn10To13, isbn13To10, isbnEquivalents, normalizeIsbn } = require('./isbn');

/**
 * Ті самі тест-кейси, що `src/lib/isbn.test.ts` — навмисно: цей файл — часткова дублікація
 * оригіналу (докладніше — коментар у `isbn.js`), тож тест підтверджує, що копія РЕАЛЬНО
 * поводиться ідентично оригіналу на тих самих прикладах, а не лише "виглядає схоже".
 */
...
describe('isbnEquivalents', () => {
  it('на вході ISBN-10 — обчислює відповідний ISBN-13', () => {
    expect(isbnEquivalents('0-306-40615-2')).toEqual({ isbn10: '0306406152', isbn13: '9780306406157' });
  });

  it('на вході ISBN-13 (978…) — обчислює відповідний ISBN-10', () => {
    expect(isbnEquivalents('9780306406157')).toEqual({ isbn10: '0306406152', isbn13: '9780306406157' });
  });

  it('на вході ISBN-13 (979…) — ISBN-10 відсутній за визначенням', () => {
    expect(isbnEquivalents('9791234567896')).toEqual({ isbn10: null, isbn13: '9791234567896' });
  });
